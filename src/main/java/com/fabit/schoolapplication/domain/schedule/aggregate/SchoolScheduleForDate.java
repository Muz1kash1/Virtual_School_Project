...
public class SchoolScheduleForDate {
   ...
   */
  public void scheduleLesson(SchoolClassName schoolClassName, int lessonNumber,
      Discipline discipline,
      TeacherId teacherId) {
    // нахождение целевого учебного дня с классом
    LessonsForSchoolClass lessons = lessonsForSchoolClasses
        .stream()
        .filter(lessonsForSchoolClass ->
            schoolClassName.equals(lessonsForSchoolClass.getSchoolClassName()))
        .findFirst()
        .orElse(null);
  ...
  /**
   * Данный метод позволяет проверить урок на факт дублирования в других классах, так как на одном
   * уроке, в одно время, может присутствовать только 1 школьный класс.
   *
  ...
  private boolean isLessonFree(LessonInSchedule lesson) {
    return lessonsForSchoolClasses
        .stream()
        .anyMatch(lessonsForSchoolClass ->
            lessonsForSchoolClass.getLessonsInScheduleList().contains(lesson));
  }

  /**
   * Данный метод проверяет наличие номера урока в классе. Метод применяется перед выставлением
   * урока в расписание.
   *
   * @param lessonNumber    порядковый номер урока
