...
public class LoadedHomeworkForClassEventTest {
  ...
  void HomeworkForClassCreatedEventTest() {
    HomeworkForClass homeworkForClass = HomeworkForClass.of(Discipline.ALGEBRA,
        LocalDate.of(2000, 2, 2),
        SchoolClassId.of(1L), HomeworkForClassId.of(1L));
    HomeworkForClass homeworkForClassContent = (HomeworkForClass) HomeworkForClass.domainEvents.get(
        0).getContent();
    ...
    Assertions.assertEquals(homeworkForClass.getId().getValue(),
        homeworkForClassContent.getId().getValue());
    Assertions.assertEquals(homeworkForClass.getSchoolClassId().getValue(),
        homeworkForClassContent.getSchoolClassId().getValue());

    HomeworkForClass homeworkForClass1 =
        HomeworkForClass.of(Discipline.BIOLOGY, LocalDate.of(2000, 2, 2), SchoolClassId.of(2L),
            HomeworkForClassId.of(2L));
    HomeworkForClass homeworkForClassContent1 = (HomeworkForClass) HomeworkForClass.domainEvents.get(
        1).getContent();
    ...
    Assertions.assertEquals(homeworkForClass1.getSchoolClassId().getValue(),
        homeworkForClassContent1.getSchoolClassId().getValue());
    Assertions.assertEquals(homeworkForClass1.getId().getValue(),
  ...
  void HomeworkTaskSetEventTest() {
    HomeworkForClass homeworkForClass = HomeworkForClass.of(Discipline.COMPUTING,
        LocalDate.of(2000, 2, 2), SchoolClassId.of(1L), HomeworkForClassId.of(1L));
    HomeworkForClass homeworkForClassContent = (HomeworkForClass) HomeworkForClass.domainEvents.get(
        0).getContent();
